...
- 不知道报名还剩多久（无倒计时）。
- 不能一眼区分「报名中 / 即将截止 / 已截止 / 已结束 / 已颁奖」。
- 报名成功后没有明确的「玩家已报名」状态，刷新后也无法确认自己是否报过。
...
| 无本地报名记录 | 仅 `erp14-player-name` 存玩家名，不存报名了哪些活动 | 刷新后无法识别「玩家已报名」 |
...
- 跨设备同步报名状态。
- 国际化、大规模重构、复杂权限管理。
- 新增 `awardedAt`、展示截止时间等字段或任何自动下架逻辑。
- 使用 `updatedAt` / `rewardDate` 推测颁奖时间或自动隐藏已颁奖卡片。
- 后台自动把 `status` 改为「已颁奖」（由保存排名结果驱动，前台自动显示，无需手动改 status）。
...
| 状态：常驻活动 | `常驻` |
| 状态：已有排名结果（results 存在且长度 > 0） | `已颁奖` |
| 状态：明确结束（无 results） | `已结束` |
| 状态：超过 24 小时可报 | `报名中` |
| 状态：≤ 24 小时可报 | `即将截止` |
| 状态：截止未结束 | `报名已截止` |
| 未报名且可报 | `立即报名 →`（可点击） |
...
| 优先级 | 条件 | 状态 |
| --- | --- | --- |
| 1 | `inferActivityType(item) === 'fixed'`（常驻活动） | `常驻` |
| 2 | `item.results` 存在且长度 > 0（已有排名结果） | `已颁奖` |
| 3 | `item.status` 含「结束」或「已结束」（且无 results） | `已结束` |
| 4 | `eventEndAt` 已过（`new Date(eventEndAt).getTime() < Date.now()`）且无 results | `已结束` |
...
- 「已颁奖」与「已结束」是两个**明确区分**的状态，不是别名：
  - `已颁奖`：活动已有排名结果（`item.results` 存在且长度 > 0）。后台保存排名结果后，前台**自动**显示「已颁奖」；管理员**不需要**手动把 `status` 改成「已颁奖」。
  - `已结束`：活动结束但**没有**排名结果，或管理员明确设置 `status` 为结束。
  - 有排名结果的状态（已颁奖）优先于普通 `status` 状态（优先级 2 高于 3、7）。
- 已颁奖卡片**不**按时间自动消失：只要活动保持 `published`，已颁奖卡片继续显示；管理员需在后台手动取消发布或删除，卡片才从玩家前台消失。
- 本轮**不**新增 `awardedAt`、展示截止时间等字段，也**不**使用 `updatedAt` 推测颁奖时间或自动隐藏。后台保存排名结果后前台即显示「已颁奖」。
...
2. 状态徽章改用扩展后的 `getActivityStatusBadge(item)`，覆盖 常驻 / 已颁奖 / 已结束 / 报名已截止 / 即将截止 / 进行中 / 报名中。
...
| 函数 | 职责 |
| --- | --- |
| `getSignupTimeState(item, now)` | 纯函数。返回 `{ state: '常驻' \| '已颁奖' \| '已结束' \| '报名已截止' \| '即将截止' \| '进行中' \| '报名中', remainingMs: number }`。完整状态集合共 7 个，基于 `signupDeadline` / `eventEndAt` / `results` / `status` 计算，不操作 DOM。 |
...
报名接口的成功判定以 `fetchWithFallback` 的**返回结果**为准，不读取任何 `success` 字段：

- `fetchWithFallback`（内部调用 `fetchJson`）在 `response.ok` 为 false 时抛出异常，进入 `submitEventSignup` 的 `catch`；HTTP 非 2xx（含 409 重复报名）都走 `catch`，**不会**写本地记录、**不会**关闭弹窗。
- `fetchWithFallback` **成功返回且未抛异常**，即视为报名接口成功。此时拿到的 `data` 是 `fetchJson` 解包后的对象（形如 `{ id, eventId, playerName, count }`），通常**没有** `success` 字段，也**不**新增对 `success` 字段的判断。
- 只有 `fetchWithFallback` 成功返回后，才能：
  - 写入本地记录（`saveLocalEventSignup`）；
  - 关闭弹窗（`closeEventSignup`）；
  - 显示成功 toast（`报名成功！你已报名「活动名称」`）；
  - 让按钮变为「玩家已报名」。
- **不**修改 `fetchJson()` 公共封装，**不**新增 `success` 字段判断；不把重复报名错误自动当作成功，不自动写入本地记录。
...
3. 剩余 < 24 小时 → 状态 `即将截止`。
4. 截止时间已过 → 状态 `报名已截止`。
5. `status` 含「已结束」且无 `results` → 状态 `已结束`。
6. 已有 `results`（长度 > 0） → 状态 `已颁奖`（不再映射为「已结束」）。
7. `status` 含「进行」/「开始」且无 `results` → 状态 `进行中`。
8. `results` 存在时优先于普通 `status`：即使 `status` 含「进行」/「开始」或「结束」，只要 `results.length > 0` 仍返回 `已颁奖`。
9. 无有效截止时间 → 不显示倒计时（不创建时间）。
10. 倒计时格式：天/小时形式正确（`还剩 X天 X小时`）。
11. 倒计时格式：HH:MM:SS 形式正确（补零）。
12. 未报名且可报 → 显示 `立即报名`。
13. 本地成功记录存在 → 显示 `玩家已报名`。
14. 不同玩家名称不能误判为已报名（activePlayerName 不匹配时返回未报名）。
15. 报名失败不写 localStorage（catch 分支不调用 `saveLocalEventSignup`）。
16. 报名成功（`fetchWithFallback` 成功返回、未抛异常）后才写 localStorage；不读取 `data.success` 字段。
17. localStorage 损坏时页面不报错（`readLocalEventSignups` 返回 `{}`）。
18. 定时器不会重复创建（`startSignupCountdownTimer` 多次调用只保留一个 `setInterval`）。
19. 不每秒重绘整个活动区域（`updateSignupCountdownNodes` 只更新 `[data-countdown]` 节点）。
20. 已颁奖且 `published` 的活动仍参与前台渲染（`renderActivitySignups` 不跳过），不因时间经过自动消失。
21. 不存在按 `updatedAt` / `rewardDate` 自动隐藏已颁奖卡片的逻辑（前台只按 `published` 决定是否展示）。
22. 现有活动测试继续通过（`test-activity-content` / `test-activity-overflow` / `test-activity-merge` / `test-activity-delete`）。
23. 320 / 375 / 414 手机视口无横向溢出。
24. 控制台错误为 0，报名弹窗在手机上完整显示。
...
- 刷新页面后本设备「玩家已报名」保留。
- `results` 存在时自动显示「已颁奖」，且不需要手动修改 `status`。
- 重新渲染（`renderActivitySignups`）后仍显示「已颁奖」。
- `published` 的已颁奖活动不因时间经过自动消失。
- 不存在按 `updatedAt` / `rewardDate` 自动隐藏的逻辑。
- 亮色 / 暗色主题均清晰。
- 控制台 0 错误。
...
6. **已颁奖卡片不自动隐藏**：已颁奖状态不启动自动隐藏倒计时，也不根据 `results` 生成时间、`rewardDate` 或 `updatedAt` 自动下架。前台只依据 `published` 决定是否继续展示；管理员需在后台手动取消发布或删除，已颁奖卡片才从玩家前台消失。本轮不新增任何自动下架功能。
