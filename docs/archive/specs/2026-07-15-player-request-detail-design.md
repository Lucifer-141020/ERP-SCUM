...
>
> **废弃说明**
> - 2026-07-16 用户重新确认不需要"查看详情 / 收起详情"。
> - 原卡片内展开详情方案不再代表当前实现。
> - **当前最终方案：**
>   - 前台直接展示建议正文；
>   - 管理员回复、完成说明、拒绝原因直接展示；
...
>   - 后台建议图片支持点击大图查看；
>   - 不修改后端、数据库和生产页。
> - 当前实现以 `frontend/js/main.js`、`frontend/css/main.css`、`scripts/test-request-detail.js`、更新日志最新记录为准。
> - **不得继续把旧 32 项详情展开要求写成"当前待实现任务"。**
>
> 历史正文保留作为决策过程记录，不再代表当前实现方向。
