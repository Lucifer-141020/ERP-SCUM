...
test('红灯-39：进行中且截止>24h 仍必须渲染倒计时节点', () => {
  const html = buildRenderSandbox({
    updates: [{ id: 14, title: '进行中活动', activityType: 'signup', status: '进行中', signupEnabled: true, signupDeadline: future48h }]
  }).renderActivitySignups();
...
// 用健壮提取器从真实源码提取 async function name(..., options = {}) 的函数体。
function extractFnRobust(src, name) {
  ...
  // 跳过参数列表中的括号（含字符串内的），定位到真正的函数体起始
  let pDepth = 0, inS = null, esc = false;
  let afterParen = parenStart;
  for (let j = parenStart; j < src.length; j++) {
    ...
    if (ch === '(') pDepth++;
    if (ch === ')') { pDepth--; if (pDepth === 0) { afterParen = j + 1; break; } }
  }
  // 找到函数体的第一个 {
  const fnOpen = src.indexOf('{', afterParen);
  // 花括号计数匹配函数体
  let bDepth = 0; inS = null; esc = false;
  for (let j = fnOpen; j < src.length; j++) {
    const ch = src[j];
    if (inS) { if (esc) { esc = false; continue; } if (ch === '\\') { esc = true; continue; } if (ch === inS) { inS = null; } continue; }
    if (ch === '"' || ch === "'" || ch === '`') { inS = ch; continue; }
    if (ch === '{') bDepth++;
    if (ch === '}') { bDepth--; if (bDepth === 0) return src.slice(start, j + 1); }
  }
  throw new Error('未匹配到结束括号');
}

test('红灯-45：FALLBACK_MAP 未定义时 fetchWithFallback 抛出原始请求错误', async () => {
  // 从真实源码提取两个函数
  const fetchJsonSrc = extractFnRobust(MAIN_JS, 'fetchJson');
  const fallbackSrc = extractFnRobust(MAIN_JS, 'fetchWithFallback');
  // 沙箱：mock fetch 返回 500，不定义 FALLBACK_MAP
  const code = `
    async function fetch(url, options) {
      return { ok: false, status: 500, json: async () => ({ message: 'HTTP 500: 内部错误' }) };
    }
    ${fetchJsonSrc}
    ${fallbackSrc}
  `;
  const factory = new Function(code + ' return { fetchWithFallback, fetchJson };');
  const sandbox = factory();
  try {
    await sandbox.fetchWithFallback('/api/events/test');
    throw new Error('fetchWithFallback 应抛出而非返回');
  } catch (err) {
    assert.ok(
      !err.message.includes('FALLBACK_MAP') && !err.message.includes('not defined'),
      '原始错误被 FALLBACK_MAP 未定义的 ReferenceError 覆盖：' + err.message
    );
    assert.ok(
      err.message.includes('HTTP 500'),
      '原始错误信息未保留，实际：' + err.message
    );
  }
});
